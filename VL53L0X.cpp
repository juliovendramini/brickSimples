...
VL53L0X::VL53L0X(PortaI2C porta)
...
bool VL53L0X::init(bool io_2v8)
{
  ...
  this->address = 0x30; 
  this->readReg(0x00); // tenta ler qualquer registro para verificar se existe um dispositivo configurado com 0x30
  if(this->last_status != 0){ //0 é OK
    this->address = ADDRESS_DEFAULT; // volta para o endereço padrão
    this->readReg(0x00); // tenta ler qualquer registro para verificar se existe um dispositivo
    if(this->last_status == 0){
      this->setAddress(0x30); // configura para o novo endereço
    }
  }
  // check model ID register (value specified in datasheet)
  temp = readReg(IDENTIFICATION_MODEL_ID);
  if (temp != 0xEE) {
      Serial.print(F("VL53L0X não encontrado na porta "));
      Serial.println(this->descricaoPorta);
      while(1);
  }else{
      Serial.print(F("VL53L0X encontrado na porta "));
      Serial.println(this->descricaoPorta);
  }
